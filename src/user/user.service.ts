...
export class UserService {
  ...
  async create(createUserDto: CreateUserDto) {
    const { email, phoneNumber, username, password, gender, role } = createUserDto;
  
    const emailVerificationToken = this.jwtService.sign({ email });
  ...
  async findAll(requestedRole?: Role) {
    if (requestedRole && ![Role.Admin, Role.Superadmin].includes(requestedRole)) {
